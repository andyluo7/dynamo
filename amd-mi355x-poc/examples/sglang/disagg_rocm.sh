...
# Required (set via env or edit before running):
#   PREFILL_NODE, DECODE_NODE: SSH-reachable hostnames of the two MI355X nodes
#   PREFILL_IP:                IP of PREFILL_NODE on the mgmt network
# Tunables (override via env): DYNAMO_VERSION, HF_CACHE, IMAGE, MODEL, TP
: "${PREFILL_NODE:?set PREFILL_NODE=<prefill-hostname>}"
: "${DECODE_NODE:?set DECODE_NODE=<decode-hostname>}"
: "${PREFILL_IP:?set PREFILL_IP=<prefill-mgmt-ip>}"
