...
CONTAINER_NAME=dynamo-sglang-poc
IMAGE=docker.io/rocm/sgl-dev:v0.5.10.post1-rocm720-mi35x-20260503
HF_CACHE=/shared/amdgpu/home/anluo/.cache/huggingface
DSR1_LOCAL=deepseek-ai/DeepSeek-R1-0528  # use HF ID; weights cached at $HF_CACHE/hub/models--...
LAUNCH_LOG=/shared/amdgpu/home/anluo/dynamo-poc/phase1-launch.log
...
class nixl_agent:
    def __init__(self, *a, **k):
        raise RuntimeError("nixl stub (AMD agg PoC)")
...
echo "[$(date)] installing ai-dynamo (no-deps) + Python 3.10 typing.Self shim"
...
# sgl-dev:v0.5.11-rocm720-mi35x-20260511 ships Python 3.10. ai-dynamo 1.1.1 uses
# `from typing import Self` (added in 3.11). Inject Self from typing_extensions
# via a .pth file — processed by site.py at startup, BEFORE any user imports.
...
PTHEOF
echo "[$(date)] typing.Self .pth shim installed"
