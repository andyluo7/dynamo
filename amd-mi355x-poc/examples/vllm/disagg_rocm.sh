#!/bin/bash
# Phase 4 escalation: vLLM 1P1D disagg with MiniMax-M2.5 + RIXL/UCX-ROCm.
# M2.5 is 229B FP8 MoE (~216 GB); needs TP=4 per node (uses 4 of 8 GPUs).
set -e
...
echo "[$(date)] === Phase 4-M25: vLLM 1P1D disagg ($MODEL TP=$TP) ==="

for n in $PREFILL_NODE $DECODE_NODE; do
    ssh -o StrictHostKeyChecking=no $n "podman rm -f dynamo-vllm-prefill dynamo-vllm-decode dynamo-prefill dynamo-decode 2>&1 | tail -2" || true
done

cat > /tmp/inside_p4_m25.sh <<'INNER'
#!/bin/bash
set -e
ROLE=$1
...
# apt + install rdma-core/iproute2
...
# LD_PRELOAD interposer (strips IBV_ACCESS_REMOTE_ATOMIC)
...
if [[ "$ROLE" == "prefill" ]]; then
    ...
    sleep 3
    echo "[$(date)] launching vLLM PREFILL worker (TP=$TP)"
    ...
    echo "[$(date)] launching vLLM DECODE worker (TP=$TP)"
...
echo "[$(date)] waiting up to 30 min for readiness (M2.5 = ~5 min NFS load each node)"
for i in $(seq 1 180); do
    sleep 10
    if ! kill -0 $W_PID 2>/dev/null; then
        echo "[$(date)] worker died ~${i}0s; tail:"
...
sleep 7200
INNER
chmod +x /tmp/inside_p4_m25.sh
scp -o StrictHostKeyChecking=no /tmp/inside_p4_m25.sh ${PREFILL_NODE}:/tmp/inside_p4_m25.sh
scp -o StrictHostKeyChecking=no /tmp/inside_p4_m25.sh ${DECODE_NODE}:/tmp/inside_p4_m25.sh
...
COMMON_FLAGS="--device /dev/kfd --device /dev/dri --device /dev/infiniband \
  ...
  -v /tmp/inside_p4_m25.sh:/tmp/inside_p4_m25.sh:ro \
...
ssh -o StrictHostKeyChecking=no $PREFILL_NODE "podman run -d --replace --name dynamo-vllm-prefill $COMMON_FLAGS --entrypoint bash $IMAGE /tmp/inside_p4_m25.sh prefill"

echo "[$(date)] launching DECODE on $DECODE_NODE"
ssh -o StrictHostKeyChecking=no $DECODE_NODE "podman run -d --replace --name dynamo-vllm-decode $COMMON_FLAGS --entrypoint bash $IMAGE /tmp/inside_p4_m25.sh decode"
