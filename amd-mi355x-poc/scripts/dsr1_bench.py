...
def send(i, n=64):
    t0=time.time()
    r=requests.post(URL, json={"model":MODEL,"messages":[{"role":"user","content":f"Tell me a brief fact ({i}). Reply in 1 sentence."}],"max_tokens":n,"temperature":0.7}, timeout=180)
    dt=time.time()-t0
...
for conc in [1,4,8]:
    ...
    if not ok:
        print(f"{conc:>4} {N:>3} FAIL"); print("err:", results[0].get("err")); continue
