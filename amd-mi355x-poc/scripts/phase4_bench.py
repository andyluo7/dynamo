...
def send(i, n=64):
    t0=time.time()
    r=requests.post(URL, json={"model":MODEL,"messages":[{"role":"user","content":f"Tell me a quick fact ({i})."}],"max_tokens":n,"temperature":0.7}, timeout=120)
    dt=time.time()-t0
...
print("warmup...")
for i in range(3):
    r=send(i,32); ok=r["ok"]; ms=r["ms"]
    print(f"  warmup {i}: ok={ok} ms={ms:.0f}")
print()
...
for conc in [1,4,8]:
    ...
    if not ok:
        print(f"{conc:>4} {N:>3} FAIL"); continue
