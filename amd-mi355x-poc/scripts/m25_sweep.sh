#!/bin/bash
# Concurrency sweep for M2.5 vLLM+RIXL disagg.
# Mirror of test12_sweep.sh but for MiniMaxAI/MiniMax-M2.5.

set -uo pipefail
RESULTS=/tmp/m25_sweep_results.csv
...
# Use a temp bench python that targets MiniMax model name
cat > /tmp/m25_test_bench.py <<'PYEOF'
...
def main():
    ...
    print(f"=== M2.5 sweep: ISL={args.isl} OSL={args.osl} c={args.conc} ===")
    ...
    if not ok:
        print("ALL FAILED"); print("err:", results[0].get("err") if results else "")
        return
...
for c in 1 4 8 16 32 64 128; do
    ...
    out=$(timeout 1200 python3 /tmp/m25_test_bench.py --isl 1024 --osl 1024 --conc $c --num-prompts $n --warmup $w 2>&1)
    echo "$out" >&2 | tail -15
